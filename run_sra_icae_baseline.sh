...
if [ "$#" -lt 2 ]; then
  echo "Usage: bash run_sra_icae_baseline.sh MODEL_PATH LORA_PATH [DATASET] [METHOD] [OUTPUT_NAME] [extra runner args...]"
  echo "Example: bash run_sra_icae_baseline.sh /mnt/hdfs/wangchangyue/LLM/Qwen3-8B /mnt/hdfs/wangchangyue/500xCompressor/output/ICAE_finetune-Qwen3-8B/checkpoint_best/pytorch_model.bin champ golden_skill"
  exit 2
fi
...
MODEL_NAME="$(basename "${MODEL_PATH}")-icae-nm${NUM_MEM}"
